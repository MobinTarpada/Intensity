...
namespace FitnessCenter.BAL
{
    public class UserSchemeMasterController
    {
        ...
        public static SchemeMaster GetUserSchemeMasterByID(long UserSchemeId)
        {
            try
            {
                return new FitnessCenterEntities().SchemeMasters.FirstOrDefault(x => x.isDeleted == false && x.ID == UserSchemeId);
            }
            catch (Exception ex)
            {
        ...
        public void DeleteUserScheme(int UserShemeID)
        {
            try
            {
                using (var context = new FitnessCenterEntities())
                {
                    var objScheme = context.SchemeMasters.SingleOrDefault(varU => varU.ID == UserShemeID);
        ...
        public static Membership GetCancellationByMemberId(string MemberNo)
        {
            try
            {
                return new FitnessCenterEntities().Memberships.FirstOrDefault(x => x.isActive == true && x.isDeleted == false && x.MembershipNo == MemberNo);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public static SchemeMaster GetCancelBySchemeId(long SchemeID)
        {
            try
            {
